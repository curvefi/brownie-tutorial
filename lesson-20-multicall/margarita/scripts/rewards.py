...
def main():
    ...
    for i in range(registry.pool_count()):
        ...
        for _pool_index in range(
            registry.get_n_coins(_pool)[0]
        ):
...
def ape(pool, pool_index, tripool_lp):
    ...
    tripool_lp.approve(pool, tripool_bal,
            {'from': whale})
    ...
    pool_lp = load_contract(
            registry.get_lp_token(pool))
    pool_bal = pool_lp.balanceOf(whale)
    if pool_lp.allowance(whale, gauge_addr) < pool_bal:
        pool_lp.approve(gauge_addr, pool_bal,
                {'from': whale})
...
def calc_cur_value():
    ...
    for i in range(registry.pool_count()):
        ...
        _addr = registry.get_gauges(
                registry.pool_list(i))[0][0]
